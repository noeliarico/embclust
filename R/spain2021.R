...
weather <- results %>%
  as_tibble() %>%
  select(fecha, provincia, tmed, tmin, tmax) %>%
...
print(weather %>% group_by(provincia) %>% count(), n = 52)
table(complete.cases(weather)) # 116 missing
print(weather[!complete.cases(weather),], n = 116)
weather <- weather %>% drop_na() # drop 116
print(weather %>% group_by(provincia) %>% count(), n = 52)
...
print(for_distance, n = 52)

fd <- for_distance[complete.cases(for_distance),-1]
n <- for_distance[complete.cases(for_distance),] %>% pull(provincia)

s <- sim_emb_matrix(fd, emb_lk, mean, dist = T)
attr(s, "Labels") <- n
d <- 1-s
